...
## Latest Verification Log (2026-03-03, Asia/Taipei)

Validation run for current PR changes:
...
- `node --check` passed:
  ...
  - `scenarios/reconnect-wave.js`
  - `lib/config.js`
- `k6 inspect` passed for all 3 scenarios (`large-room`, `many-rooms`, `reconnect-wave`).
- Minimal smoke execution was attempted for `large-room` with 1 member / 1 publisher and 1s+1s+1s timing.
  - `k6` executed the script, but token acquisition failed because local Keycloak endpoint was unreachable in this runtime (`connect: operation not permitted` to `localhost:8080`).
  - Runtime metrics from this attempt are not representative for performance conclusions; use a full load-test host or CI environment with reachable local stack.

## Local Runtime Verification Update (2026-03-04, Asia/Taipei)

Full local runs were attempted and surfaced runtime integration issues, then corresponding local test fixes were applied:

- Fixed local stack startup for auth callout by enabling JetStream on `AUTH` account in `nats/nats-server.conf`.
- Fixed Keycloak load-test users (`loadtest-*`) so password grant works under this realm setup.
- Fixed NATS CONNECT payload in `lib/nats-ws.js`:
  - `auth_token` (not `token`)
  - `headers: true` when using `no_responders`
- Fixed scenario publish path for current permissions model:
  - `deliver.{user}.send.{room}` (replaces direct `chat.{room}` publish in scenario publishers)

Current status:

- Runtime auth/token/connectivity blockers were resolved in this local session.
- A clean full-duration pass/fail matrix still needs one final end-to-end rerun after these latest scenario transport/subject fixes.

Latest conclusion:

- 5,000-room per-user subscription setup completed in about **10.3s** in local testing.
- Full 10,000-member fanout remains unstable locally (auth/connect saturation during ramp), so a valid 10k p95/p99 latency number is still pending.
