...
|----------|--------|-----------|
| Scope | Private rooms + DMs only (enforced by UI — no server-side room-type check) | Public rooms don't need encryption; restricts complexity |
...
| Translation | Disabled in UI for E2EE rooms | translation-service receives ciphertext and lacks raw key access, so cannot translate |
...
```mermaid
sequenceDiagram
    participant A as Browser (Alice - CAS winner)
    participant N as NATS
    participant KS as e2ee-key-service

    Note over A: room.changed.engineering → {action:"leave", userId:"charlie"}
    Note over A: All online members race to bump epoch via CAS; first writer wins

    A->>A: generateRoomKey() → new AES-256-GCM key (epoch 2)

    A->>N: request("e2ee.room.epoch.engineering", {newEpoch:2})
    N->>KS: deliver
    KS->>KS: CAS update E2EE_ROOM_META["engineering"].currentEpoch = 2
    KS-->>A: {ok: true}

    Note over A: CAS succeeded — this client is the rotation coordinator.<br/>Losing clients receive {error:"conflict, retry"} and fetch the winner's key.

    A->>A: storeRoomKey("engineering", 2, newKey) locally
    ...
    A->>N: request("e2ee.roomkey.raw", {room:"engineering", epoch:2, rawKey})
